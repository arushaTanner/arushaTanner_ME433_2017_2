package com.example.brandon.greenscreen;
...
public class MainActivity extends Activity implements TextureView.SurfaceTextureListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        myTextView = (TextView) findViewById(R.id.label1);
    ...
    public void onSurfaceTextureUpdated(SurfaceTexture surface) {
        ...
        if (c != null) {
            ...
            for(int row=0; row<bmp.getHeight(); row+=5) {
                int startY = row; // which row in the bitmap to analyze to read
                bmp.getPixels(pixels, 0, bmp.getWidth(), 0, startY, bmp.getWidth(), 1);

                // in the row, see if there is more green than red
                for (int i = 0; i < bmp.getWidth(); i++) {
                    if ((green(pixels[i])>thresh)&& (green(pixels[i]) - red(pixels[i]) > 0)&&(green(pixels[i])-blue(pixels[i])>0)) {
